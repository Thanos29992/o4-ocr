...
require wget; require ovc; require python3

# ---- detection -----------------------------------------------------------
if [[ ! -f "$DEST/ch_PP-OCRv4_det_infer/inference.xml" ]]; then
  echo "== download + convert detection model =="
  ( cd "$DEST" && wget -q -O det.tar "$DET_URL" && tar xf det.tar && rm -f det.tar )
...
if [[ ! -f "$DEST/ch_PP-OCRv4_rec_server_infer/inference.xml" ]]; then
  ...
  ( cd "$DEST" && wget -q -O rec.tar "$REC_URL" && tar xf rec.tar && rm -f rec.tar )
...
if [[ ! -f "$DEST/ppocr_keys_v1.txt" ]]; then
  ...
  wget -q -O "$DEST/ppocr_keys_v1.txt" "$DICT_URL"
fi
